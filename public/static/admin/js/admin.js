...
function formSubmit(form) {
    let loadT = layer.msg('1111111111正在提交，请稍候…', {icon: 16, time: 0, shade: [0.3, "#000"]});
 ...
 */
$(function () {
    $('body').on('click', '.AjaxButtonDelete', function (event) {
        event.preventDefault();
        ...
        var url = $(this).data("url");
        //访问方式，默认post
        var layerMethod = $(this).data("method") || 'DELETE';
        //访问成功后跳转的页面，不设置此参数默认根据后台返回的url跳转
        var go = $(this).data("go") || 'url://reload';
